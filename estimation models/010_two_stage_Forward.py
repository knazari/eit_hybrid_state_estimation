# ============================================================
# eit_two_stage_physics_final_ae.py  (Autoencoder-G version; BP-based σ)
# ------------------------------------------------------------
...
# - Stage B:
#     F:        v_s -> (x,y,F)_s, with physics consistency via:
#                y_pred -> R -> Dec -> sigma_s -> F_hat -> v_s (match xb)
...
protocol = create_protocol(
    ...
    parser_meas=PARSER_MEAS,     # <<< same parser as real-time
)
_ = EITForward(mesh, protocol)   # not used further, but OK to keep

# --- BP inverse (same as real-time, but no display scaling) ---
bp_inv = BP(mesh, protocol)
bp_inv.setup(weight="none")      # <<< matches your live script

def recon_sigma_bp(v1, v0):
    # IMPORTANT: normalize=True, log_scale=False as in your live code
...
def bp_node_to_element(ds_node: np.ndarray, mesh_obj) -> np.ndarray:
    """
    Convert BP output defined on nodes (len = n_nodes) to per-element
    values (len = n_elements) by averaging the triangle's node values.
    """
    # mesh.element is [n_elem, 3] of node indices
...
for i in range(V.shape[0]):
    ...
    ds = recon_sigma_bp(v1, v0)           # <<< BP-based, normalized (no 192.0)
    # If BP returned node-wise values, map to elements
...
class CNN1D_Head(nn.Module):
    """Voltage -> (x,y,force) model (expects [N, 1, L])"""
...
# Freeze ZReg for Stage B (you can also fine-tune jointly later)
...
# ===========================
F_hat = MLP(in_dim=Sigma_tr_s.shape[1], hidden=[256,128], out_dim=V.shape[1]).to(device)
...
# ===========================
# Stage B: voltage -> (x,y,force) with physics loss
# ===========================
X_F_tr = V_tr_s_t[:, None, :]   # [N,1,L]
Y_F_tr = Y_tr_s_t               # [N,3]
X_F_te = V_te_s_t[:, None, :]
Y_F_te = Y_te_s_t

F = CNN1D_Head(input_len=V.shape[1], out_dim=3).to(device)
opt_F = optim.Adam(F.parameters(), lr=1e-3)
mse = nn.MSELoss()
...
print("\n=== Stage B: train F (voltage -> state) with physics consistency ===")
for ep in range(EPOCHS_B):
    ...
    for i in range(0, NtrB, BATCH_B):
        ...
        z_pred   = ZReg(y_pred)
        sigma_s  = dec(z_pred)
        v_sim_s  = F_hat(sigma_s)
        v_true_s = xb.squeeze(1)
    ...
    if (ep % 25) == 0 or ep == EPOCHS_B - 1:
        ...
        with torch.no_grad():
            ...
            z_val   = ZReg(y_val)
            sigma_v = dec(z_val)
            phys_v  = mse(F_hat(sigma_v), X_F_te.squeeze(1)).item()
...
print(f"\n✅ Finished. BP-based reconstruction aligned with real-time. Artifacts in '{OUT_DIR}/'.")
